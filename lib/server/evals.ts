...
export function validateSet(raw: unknown): EvalSet | string {
  const s = raw as { id?: string; agent?: string; name?: string; model?: string; cases?: unknown[] };
  ...
  for (let i = 0; i < s.cases.length; i++) {
    ...
    if (!checks.length) return `case ${i} needs at least one check`;
  ...
  return { id, agent: s.agent, name: String(s.name ?? id), model: String(s.model ?? ""), cases };
}
...
/** Grade one finished run against a case's checks. Deterministic on purpose. */
export function grade(
  checks: Check[],
  runState: string,
  result: unknown,
): { passed: boolean; detail: { kind: string; ok: boolean; note: string }[] } {
  const blob = JSON.stringify(result ?? "").toLowerCase();
  const detail = checks.map((ch) => {
    if (ch.kind === "state") {
  ...
  return { passed: detail.every((d) => d.ok), detail };
}
